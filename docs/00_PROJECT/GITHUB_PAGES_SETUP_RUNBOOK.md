...
2026-03-20 hardening 完了:
- `docs/site/favicon.svg` を追加し、全 HTML に `rel="icon"` を設定済み
- `pages.yml` に `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` を設定済み
- `pages.yml` の action は公式 tag のフル SHA pin に更新済み

## Playwright 証跡

### 1. 有効化前の判断

初回確認では公開 URL は `Site not found · GitHub Pages` で、`Settings > Pages` でも Pages 有効化条件が満たされていなかった。  
そのため、repository を `public` にし、`Source = GitHub Actions` へ切り替える方針を採用した。

### 2. Pages の Source を GitHub Actions に切り替えた
...
### 3. 失敗済み Pages workflow を再実行して成功させた
...
対応内容:
- `docs/site/favicon.svg` を追加した
- 全 HTML の `<head>` に `rel="icon"` を追加した

確認結果:
- `https://knoguchi-ship-it.github.io/kaigo-ide/favicon.svg?v=aa6a72f` は `200 OK`

### 2. GitHub Actions の Node 24 / SHA pin 対応

対応内容:
- `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` を設定した
- `actions/checkout` を Node 24 対応版へ更新した
- `configure-pages` / `upload-pages-artifact` / `deploy-pages` を含め、Pages workflow の action をフル SHA pin にした

確認結果:
- `https://knoguchi-ship-it.github.io/kaigo-ide/?v=aa6a72f` は更新済み HTML を返した
- `GitHub Pages 公開中` と `運用ページの情報粒度調整` の文言を live HTML で確認した
